...
bool readCpuTimes(CpuTimes &times) {
  std::ifstream fileWithStats(LINUX_STAT);
...
bool readRamParts(RamParts &parts) {
  std::ifstream fileWithMemory(LINUX_MEM);
  ...
  if (allMemory.count("MemAvailable")) {
    ...
    availableKb =
        allMemory["MemFree"] + allMemory["Buffers"] + allMemory["Cached"];
  }
  ...
  double usedPercentTms =
      (totalInBytes == 0) ? 0.0
                          : (double)usedInBytes * 100.0 / (double)totalInBytes;
...
std::string getUptime() {
  std::ifstream fileWithUptime(LINUX_UPTIME);
  if (!fileWithUptime.is_open()) {
    return "Failed to open '" + LINUX_UPTIME "'\n";
  }
  double timeInSec;
  ...
  std::string finalString = std::to_string(days) + "d" + std::to_string(hours) +
                            "h " + std::to_string(minutes) + "m";
  return finalString;
}

double getTemp() {
  std::ifstream fileWithTemp(LINUX_TEMP);
